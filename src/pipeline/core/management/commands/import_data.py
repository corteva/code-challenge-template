...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if kwargs.get(WEATHER, False):
            self.stdout.write(
                self.style.SUCCESS(
                    f"Starting ingestion of weather data from the txt files in the wx_data folder at {start_time}."
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"Failed start data ingestion. Please specify -w/--weather or -y/--yield after 'import_data' in the command."
                )
            )
        return
